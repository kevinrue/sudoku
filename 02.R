...
compute_cell_choices <- function(.grid, .row, .column) {
  grid_value <- .grid %>% 
    filter(row == .row & column == .column) %>% 
    pull(value)
  if (!is.na(grid_value)) {
...
compute_cell_choices(sudoku_grid, 1, 3)
compute_cell_choices(sudoku_grid, 2, 4)
compute_cell_choices(sudoku_grid, 3, 8)
compute_cell_choices(sudoku_grid, 6, 8)
compute_cell_choices(sudoku_grid, 8, 7)
compute_cell_choices(sudoku_grid, 1, 2)
  ...
  for (.row in 1:9) {
    for (.column in 1:9) {
      ...
      if (!is.na(sudoku_grid_value)) {
        ...
        choices <- compute_cell_choices(sudoku_grid, .row, .column)
